...
RSpec.describe 'Create New User', type: :feature do
  describe 'When user visits "/register"' do
    before(:each) do
      ...
      @user = User.create!(name: 'Sam', email: 'sam@email.com', password: 'sam12345', password_confirmation: 'sam12345')
    ...
    it 'When they fill in the form with their name and email then they are taken to their dashboard page "/users/:id"',
       :vcr do
      fill_in 'user[name]', with: 'Chris'
      fill_in 'user[email]', with: 'chris@email.com'
      fill_in 'user[password]', with: 'chris123'
      fill_in 'user[password_confirmation]', with: 'chris123'
    ...
    it 'when they fill in form with information, email (non-unique), submit, redirects to user show page' do
      ...
      fill_in 'user[password]', with: 'tommy123'
      fill_in 'user[password_confirmation]', with: 'tommy123'
    ...
    it 'when they fill in form with missing information' do
      fill_in 'user[name]', with: 'Hoa'
      fill_in 'user[email]', with: 'hoadam@gmail.com'
      fill_in 'user[password]', with: ''
      fill_in 'user[password_confirmation]', with: ''
      click_button 'Create New User'
      expect(current_path).to eq(register_user_path)
      expect(page).to have_content("Password can't be blank, Password confirmation can't be blank")
    end
